    ...
    return (
      <div>
        <h2 id="comment-heading">
          <Translate contentKey="xprojectApp.comment.home.title">Comments</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="xprojectApp.comment.home.createLabel">Create new Comment</Translate>
          </Link>
        </h2>
            ...
            <tbody>
              {commentList.map((comment, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <TextFormat type="date" value={comment.datetime} format={APP_DATE_FORMAT} />
                  </td>
                  <td>{comment.sub ? <Link to={`comment/${comment.sub.id}`}>{comment.sub.id}</Link> : ''}</td>
                  <td>{comment.comments ? <Link to={`product/${comment.comments.id}`}>{comment.comments.id}</Link> : ''}</td>
